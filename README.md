...
A collaborative filtering recommendation system uses explicit rstings data given by each user (in this case, a numeric score 1-5), makes each user and their ratings into a vector, thereby creating a sparse matrix containing one row for each user, with columns for each piece of content. When one of the users wants a recommendation, we calculate a similarity metric between the requesting user and all other users. We then choose the modt similar user to the requestor, and return recommendations consisting of the books that most similar user had read, that our requestor hasn't. in this case, I use cosine similarity to find the most similar user (the closest vector). 
As in the content-based recommender, I build a function that takes in a user's ID, calculated similarity scores, and returns the desired number of recommendations. In the interest of saving computing power and time, instead of calculating a similarity matrix containing scores for every single user against every other single user, I calculated only scores of the requesting user against all others. 
