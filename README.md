...
A collaborative filtering recommendation system first takes in explicit ratings data given by each user (in this case, a numeric score 1-5), then makes each user and their ratings into a vector, thereby creating a sparse matrix containing one row for each user and columns for each piece of content. When one of the users wants a recommendation, the recommender calculates a similarity metric between the requesting user and all other users. It then chooses the most similar user to the requestor, and return recommendations consisting of the books that the most similar user has rated but that our requestor hasn't. In this case, I use cosine similarity to find the most similar user (the closest vector). 
As in the content-based recommender, I build a function to take in user input and return recommendations. The CF function takes in a user's ID, calculates similarity scores, and returns the desired number of recommendations. In the interest of saving computing power and time, instead of calculating a similarity matrix containing scores for every single user against every other single user, I calculated only scores of the requesting user against all others. 
